...
class StockBalanceReport:
	...
	def run(self):
		...
		# Sorting by Balance Value Descending
		if self.data:
			self.data.sort(key=lambda x: x.get("bal_val", 0), reverse=True)
	...
	def get_group_by_key(self, row) -> tuple:
		branch = row.get("branch") or "Unassigned Plant"
		# Grouped by Company, Item, and Branch (Plant) only
		group_by_key = [row.company, row.item_code, branch]
	...
	def prepare_stock_ledger_entries(self):
		...
		select_fields = [
			...
			sle.stock_value_difference, sle.item_code.as_("name"), sle.voucher_no,
			sle.stock_value, sle.batch_no, sle.serial_no, sle.serial_and_batch_bundle,
			sle.has_serial_no, sle.voucher_detail_no, item_table.item_group,
			item_table.stock_uom, item_table.item_name,
		]
		if frappe.db.has_column("Stock Ledger Entry", "branch"):
			select_fields.append(sle.branch)
		query = (
			frappe.qb.from_(sle)
		...
		if self.filters.get("branch"):
			query = query.where(sle.branch == self.filters.get("branch"))
	...
	def get_columns(self):
		return [
			...
			{"label": _("Balance Value"), "fieldname": "bal_val", "fieldtype": "Currency", "width": 110, "options": "Company:company:default_currency"},
			{"label": _("Out Qty"), "fieldname": "out_qty", "fieldtype": "Float", "width": 90, "convertible": "qty"},
			{"label": _("Out Value"), "fieldname": "out_val", "fieldtype": "Currency", "width": 90},
			{"label": _("In Qty"), "fieldname": "in_qty", "fieldtype": "Float", "width": 90, "convertible": "qty"},
			{"label": _("In Value"), "fieldname": "in_val", "fieldtype": "Currency", "width": 90},
			{"label": _("Opening Qty"), "fieldname": "opening_qty", "fieldtype": "Float", "width": 100, "convertible": "qty"},
			{"label": _("Opening Value"), "fieldname": "opening_val", "fieldtype": "Currency", "width": 110, "options": "Company:company:default_currency"},
			{"label": _("Plant"), "fieldname": "branch", "fieldtype": "Link", "options": "Branch", "width": 120},
	...
	for group_by_key in iwb_map:
		...
		for key, val in qty_dict.items():
			if inventory_dimensions and key in inventory_dimensions:
				continue
			if key in [
				"item_code", "warehouse", "item_name", "item_group", "project",
				"branch", "stock_uom", "company", "opening_fifo_queue",
			]:
				continue
