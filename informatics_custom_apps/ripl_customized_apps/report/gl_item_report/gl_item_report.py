...
def get_data(filters):
    ...
    gl_entries = frappe.db.sql(f"""
        SELECT 
            gl.posting_date, gl.company, gl.account, gl.voucher_type, gl.voucher_no, gl.party,
            gl.branch as plant, gl.cost_center, gl.segment, gl.debit, gl.credit
        FROM `tabGL Entry` gl
        WHERE {where_clause}
    ...
    for v_type, v_nos in vouchers.items():
        ...
        if v_type == "Sales Invoice":
            items = frappe.db.sql("""
                SELECT parent, item_code, item_name, qty, rate, income_account as item_account
            ...
            items = frappe.db.sql(f"""
                SELECT parent, item_code, item_name, qty, rate, expense_account as item_account
            ...
            items = frappe.db.sql("""
                SELECT parent, item_code, item_name, qty, basic_rate as rate, expense_account as item_account
    ...
    for gl in gl_entries:
        ...
        matched_items = [i for i in items if i.get("item_account") == gl.account]
        ...
        if matched_items:
            for idx, item in enumerate(matched_items):
                ...
                # ✅ FIX: Always assign the actual quantity and item_value
                row.update({
                ...
                # ✅ Prevent duplication of GL totals (Debit/Credit) for multiple items
                if idx > 0:
                    row["debit"] = 0
                    row["credit"] = 0
