			...
			row.update(
				{
					...
					"final_deduction":ss.total_deduction + ss.total_loan_repayment
				}
			)

		else:
			row.update(
				{"gross_pay": ss.gross_pay, "total_deduction": ss.total_deduction, "net_pay": ss.net_pay,"final_deduction":ss.total_deduction+ss.total_loan_repayment}
			)
		if department not in data_grouped_by_department:
			data_grouped_by_department[department]=[]
		data_grouped_by_department[department].append(row)

		data = [row for rows in data_grouped_by_department.values()for row in rows]
...
def get_columns(earning_types, ded_types):
	...
	columns.extend(
		[
			...
			{
				"label": _("Final Deduction"),
				"fieldname": "final_deduction",
				"fieldtype": "Currency",
				"options": "currency",
				"width": 120,
			},
