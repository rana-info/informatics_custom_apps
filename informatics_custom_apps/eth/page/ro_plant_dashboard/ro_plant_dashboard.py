...
def get_dashboard(date=None):
    ...
    for log in logbooks:
        ...
        doc = frappe.get_doc("RO Plant Log Book", log.name)
...
def get_norms():
    ...
    for row in doc.ro_plant_log_norms:
...
def get_parameters():
    ...
    for row in doc.ro_plant_log_norms:
        ...
        parameters.append({
            ...
            "label": frappe.get_meta("RO Plant Log Book Item").get_field(row.fieldname).label,
...
def get_field_labels():
    ...
    meta = frappe.get_meta("RO Plant Log Book Item")
...
def get_plant_logbook(plant, date):
    ...
    rows = frappe.get_all(
        "RO Plant Log Book Item",
        filters={"parent": log},
        fields=["*"],
        order_by="idx"
    )

    # Create parameters from configured norms
    parameters = {}
    ...
    for fieldname, norm in norms.items():
        ...
        parameters[fieldname] = {
            ...
            "section": norm.get("section") or "Other",
            "unit": norm.get("unit", ""),
            "min": norm.get("min"),
            "max": norm.get("max"),
            "values": {}
    ...
    for row in rows:
        ...
        for fieldname in parameters:
            ...
            # Keep 0 as a valid displayed value
            if value is None:
                value = ""
